...
def main():
    ...
    teacher_models = [sys.argv[2], sys.argv[2], sys.argv[2], sys.argv[2], sys.argv[2]]
    gpu_id = sys.argv[3]
    ...
    for tch, option in enumerate(teacher_models, start=1):
        ...
        tsv_path = os.path.join(results_dir, f"{app_name}.teacher_{tch}.{option}.tsv")
        
        print("Loading data for tch:", tch)

        train_loader = torch.load(os.path.join(processed_dir, f"{app_name}.train_{tch}.pt"))
