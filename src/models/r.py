...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_block, num_classes=100, num_channel=3):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(num_channel, self.DIM, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(self.DIM),
            nn.ReLU(inplace=True))
        #we use a different inputsize than the original paper
        #so conv2_x's stride is 1
        self.conv2_x = self._make_layer(block, self.DIM, num_block[0], 1)
        self.conv3_x = self._make_layer(block, self.DIM*2, num_block[1], 2)
        self.conv4_x = self._make_layer(block, self.DIM*4, num_block[2], 2)
        self.conv5_x = self._make_layer(block, self.DIM*8, num_block[3], 2)
        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(self.DIM*8 * block.expansion, num_classes)
...
class ResNet_tiny(nn.Module):
    ...
    def __init__(self, block, num_block, num_classes=100, num_channel=3):
        ...
        self.conv4_x = self._make_layer(block, self.DIM*4, num_block[2], 2)
        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(self.DIM*4 * block.expansion, num_classes)
        self.sigmoid = nn.Sigmoid()
