...
def preprocessing(data, hardware):
    ...
    df=df.dropna()
    df['past_block_addr']=df.apply(lambda x: [item if pd.notnull(item) else 0 for item in x['past_block_addr_abs']], axis=1)
    df['past_delta']=df.apply(lambda x: [item if pd.notnull(item) else 0 for item in x['past_delta_abs']], axis=1)
...
def main():
    ...
    print("Beginning to save data for: stu no kd")
