...
def main():
    ...
    with torch.no_grad():
        torch.save(df_test, os.path.join(processed_dir,f"{app_name}.df_stu.pt"))
    ...
    with torch.no_grad():
        torch.save(test_loader_stu, os.path.join(processed_dir, f"{app_name}.test_stu.pt"))
    ...
    for tch in range(num_tch):
        ...
        with torch.no_grad():
            torch.save(train_loader, os.path.join(processed_dir, f"{app_name}.train_{tch+1}.pt"))
            torch.save(test_loader, os.path.join(processed_dir, f"{app_name}.test_{tch+1}.pt"))
            torch.save(df_test_tch, os.path.join(processed_dir, f"{app_name}.df_{tch+1}.pt"))
