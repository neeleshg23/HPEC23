...
def main():
    ...
    print("Beginning to save data for stu: single kd")
    with torch.no_grad():
        torch.save(df_test, os.path.join(processed_dir,f"{app_name}.df.pt"))    
    
    train_MAP   = MAPDataset(df_train)
    test_MAP    = MAPDataset(df_test)
   
    train_loader= DataLoader(train_MAP, batch_size=n_batch, shuffle=True, collate_fn=train_MAP.collate_fn)
    test_loader = DataLoader(test_MAP, batch_size=n_batch, shuffle=False, collate_fn=test_MAP.collate_fn)

    with torch.no_grad():
        torch.save(test_loader, os.path.join(processed_dir, f"{app_name}.test.pt"))
        torch.save(train_loader, os.path.join(processed_dir, f"{app_name}.train.pt"))
